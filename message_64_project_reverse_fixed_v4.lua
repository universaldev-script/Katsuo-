...
local function AddValue(Mapping, Name, Value, Rarity)
    ...
    local NormalizedRarity = Rarity and tostring(Rarity) or nil

    local Info = {
        Rarity = NormalizedRarity,
        Value = Value,
        Chroma = NormalizedRarity
            and NormalizedRarity:lower() == "chroma"
            or false
    }
...
local function WalkValueTable(Node, Mapping, InheritedRarity, Seen)
    ...
    -- Project Reverse may return an array of item objects, e.g.
    -- { {name="Scythe", value=123, rarity="Godly"}, ... }.
    -- The previous walker used the array index ("1", "2", ...) as the
    -- item name, which explains why 1063 values loaded but inventory
    -- names still returned Unknown.
    local ExplicitName =
        Node.name or Node.Name or
        Node.itemName or Node.ItemName or
        Node.item_name or Node.Item_Name or
        Node.id or Node.Id
    ...
    local ExplicitRarity =
        ...
        Node.tier or Node.Tier

    if ExplicitName ~= nil and ExplicitValue ~= nil then
        local Rarity = ExplicitRarity or InheritedRarity
        ...
        -- Still inspect nested fields in case the object contains
        -- additional aliases/items.
    ...
    for Name, Value in pairs(Node) do
        ...
        elseif type(Value) == "table" then
            ...
            if ChildRarity ~= nil then
                Rarity = tostring(ChildRarity)
            ...
            local ChildName =
                ...
                Value.itemName or Value.ItemName or
                Value.item_name or Value.Item_Name
            ...
            if ChildName ~= nil and ChildValue ~= nil then
                if AddValue(
                    Mapping,
                    tostring(ChildName),
                    ChildValue,
...
local function GetDisplayRarity(ItemInfo)
    if ItemInfo and ItemInfo.Rarity and ItemInfo.Rarity ~= "" then
        return ItemInfo.Rarity
    end

    return "Not supplied by Project Reverse"
end
...
local function LookupInventoryItem(ItemId, Quantity)
    ...
    local RarityName = GetDisplayRarity(ItemInfo)
    local Value = ItemInfo and tonumber(ItemInfo.Value) or 0
...
-- Automatic diagnostics for the common MM2 inventory names.
for _, DebugName in ipairs({"DefaultGun", "DefaultKnife", "Scythe"}) do
    local DebugInfo = FindItemValue(DebugName)

    if DebugInfo then
        print(string.format(
            "[MM2] MATCH %s -> ProjectReverseValue=%s rarity=%s",
            DebugName,
            tostring(DebugInfo.Value),
            tostring(DebugInfo.Rarity)
        ))
    else
        warn("[MM2] NO MATCH:", DebugName)
    end
end
