...
local function LoadDatabase()
    ...
    if not Response or type(Response.Body) ~= "string" or #Response.Body < 2 then
        warn("[MM2] Project Reverse returned an empty/invalid response.")
        return nil, nil
    end
    ...
    if not JSONOK or type(Result) ~= "table" then
        warn("[MM2] Project Reverse response is not valid JSON.")
        return nil, nil
    end

    local Records = Result.data
    if type(Records) ~= "table" then
        warn("[MM2] Project Reverse response has no data array.")
        return nil, nil
    end
    ...
    local function AddProjectReverseItem(ItemName, Value)
    ...
    for _, Record in ipairs(Records) do
        if type(Record) == "table" and
           tostring(Record.game or Record.Game or ""):lower() == "mm2" then

            local ItemName = Record.name or Record.Name or
                             Record.itemName or Record.ItemName

            local Value = Record.price or Record.Price or
                          Record.value or Record.Value

            AddProjectReverseItem(ItemName, Value)
        end
    end

    if Count == 0 or next(Mapping) == nil then
        warn("[MM2] Project Reverse decoded successfully, but no MM2 item values were found.")
        return nil, nil
    end

    warn("[MM2] Loaded " .. tostring(Count) .. " MM2 item values from Project Reverse.")

    -- Project Reverse supplies prices, not the rarity hierarchy used by
    -- the rest of this script. Keep the existing rarity priority unchanged.
    return Mapping, RarityPriority
end
