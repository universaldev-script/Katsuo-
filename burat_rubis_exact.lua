...
local function LoadDatabase()
    ...
    if not Success then
        warn("[MM2] Rubis value request failed:", Err)
        return nil, nil
    end

    if not Response or type(Response.Body) ~= "string" or #Response.Body < 2 then
        warn("[MM2] Rubis returned an empty/invalid response.")
        return nil, nil
    end
    ...
    -- Rubis returns the value database inside a Lua long-comment:
    -- --[[ return { ... } ]] return {receiver, webhook}
    -- Extract the commented database instead of loading the trailing
    -- receiver/webhook table.
    local CommentStart = Body:find("--[[", 1, true)
    local CommentEnd = CommentStart and Body:find("]]", CommentStart + 4, true)

    if not CommentStart or not CommentEnd then
        warn("[MM2] Rubis response did not contain the expected value database block.")
        return nil, nil
    end

    local DatabaseSource = Body:sub(CommentStart + 4, CommentEnd - 1)
    local Loader = loadstring(DatabaseSource)

    if not Loader then
        warn("[MM2] Could not compile the Rubis value database.")
        return nil, nil
    end

    local LoaderOK, RawDatabase = pcall(Loader)
    if not LoaderOK or type(RawDatabase) ~= "table" then
        warn("[MM2] Could not decode the Rubis value database.")
        return nil, nil
    end
    ...
    local function AddItemMapping(ItemName, RarityName, Value)
        ...
        local Rarity = tostring(RarityName or "Unknown")
        ...
        local ItemInfo = {
            Rarity = Rarity,
            Value = Value,
            Chroma = string.lower(Rarity) == "chroma"
        }
    ...
    for RarityName, ItemsList in pairs(RawDatabase) do
        if type(ItemsList) == "table" then
            local RarityText = tostring(RarityName)
            FoundRarities[RarityText] = true

            for ItemName, Value in pairs(ItemsList) do
                if type(Value) == "number" then
                    AddItemMapping(ItemName, RarityText, Value)
                elseif type(Value) == "string" then
                    local NumericValue = tonumber(Value)
                    if NumericValue then
                        AddItemMapping(ItemName, RarityText, NumericValue)
                    end
                elseif type(Value) == "table" then
                    local ItemValue = tonumber(
                        Value.Value or Value.value or
                        Value.Price or Value.price or
                        Value.val or Value.Val
                    )
                    local ItemRarity = Value.Rarity or Value.rarity or RarityText
    ...
    if next(Mapping) == nil then
        warn("[MM2] Rubis database decoded but contained no numeric item values.")
        return nil, nil
    end

    warn("[MM2] Loaded Rubis item values successfully.")
